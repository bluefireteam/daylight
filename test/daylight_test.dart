    ...
    group('DaylightCalculator', () {
      group('calculateEvent', () {
        group('sunrise', () {
          ...
          test('official', () {
            final time = calculator.calculateEvent(
                october, Zenith.official, EventType.sunrise);
            expect(DateFormat("HH:mm:ss").format(time), "22:36:33"); // UTC
          });
          test('nautical', () {
            final time = calculator.calculateEvent(
                october, Zenith.nautical, EventType.sunrise);
            expect(DateFormat("HH:mm:ss").format(time), "21:42:09"); // UTC
          });
          test('civil', () {
            final time = calculator.calculateEvent(
                october, Zenith.civil, EventType.sunrise);
            expect(DateFormat("HH:mm:ss").format(time), "22:11:36"); // UTC
          });
          test('astronomical', () {
            final time = calculator.calculateEvent(
                october, Zenith.astronomical, EventType.sunrise);
            expect(DateFormat("HH:mm:ss").format(time), "21:11:56"); // UTC
          });
          test('golden', () {
            final time = calculator.calculateEvent(
                october, Zenith.golden, EventType.sunrise);
        ...
        group('sunset', () {
          ...
          test('official', () {
            final time = calculator.calculateEvent(
                july, Zenith.official, EventType.sunset);
            expect(DateFormat("HH:mm:ss").format(time), "20:21:47"); // UTC
          });
          test('nautical', () {
            final time = calculator.calculateEvent(
                july, Zenith.nautical, EventType.sunset);
          ...
          test('astronomical', () {
            final time = calculator.calculateEvent(
                july, Zenith.astronomical, EventType.sunset);
            expect(time, null); // UTC
          });
          test('golden', () {
            final time = calculator.calculateEvent(
                july, Zenith.golden, EventType.sunset);
